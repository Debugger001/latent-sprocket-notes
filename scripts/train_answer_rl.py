#!/usr/bin/env python3
"""Train answer-only GRPO or Rank-GRPO from the archived teacher adapter."""
...
from openbench_rerank_rl.answer_rl import AnswerRLAlgorithm
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/rank_grpo_answer_only_qwen3_1p7b_a100_stable.yaml"),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help="number of fresh 8-prompt/32-completion rollout batches",
    )
    parser.add_argument(
        "--max-query-steps",
        type=int,
        help="optional prompt cap for a partial-window smoke test",
    )
    ...
    parser.add_argument(
        ...
        help="override the YAML W&B mode; use disabled for a smoke test",
    )
...
def _validate_answer_only_prompt(
    ...
    *,
    ...
    if "<think>" in prompt or "<answer>" in prompt:
        raise ValueError(
            f"{source}:{line_number}: answer-only prompt contains rubric envelope tags"
        )
...
def iter_training_examples(
    ...
    *,
    ...
    """Read materialized answer-only rows and reject prompt drift eagerly."""
        ...
        for line_number, line in enumerate(handle, start=1):
            ...
            if not isinstance(prompt, str) or not prompt:
                raise ValueError(
                    ...
                    "with --include-prompts --prompt-style answer-only"
                )
            ...
            if type(slate_k) is not int or slate_k <= 0:
                raise ValueError(f"{path}:{line_number}: k must be a positive integer")
...
def _require_canonical(
    ...
    if value != expected:
        raise ValueError(
            f"the answer-only baseline runtime requires {key}={expected!r}; "
            f"got {value!r}"
        )
...
def _validate_canonical_config(
    *,
    ...
    if name not in {"grpo", "rank_grpo"}:
        raise ValueError("algorithm.name must be 'grpo' or 'rank_grpo'")
    algorithm_name: AnswerRLAlgorithm = name
    ...
    for section, key, expected in (
        ...
        (algorithm, "rank_metric", "ndcg"),
        (algorithm, "lenient_integer_list_grading", True),
        (algorithm, "positive_first_occurrence_only", True),
        (optimization, "reference_kl_coefficient", 0.01),
        (optimization, "learning_rate", 5e-6),
        (optimization, "weight_decay", 0.0),
        (optimization, "effective_prompt_batch_size", PROMPTS_PER_ROLLOUT_STEP),
        (optimization, "effective_original_batch_size", ORIGINALS_PER_ROLLOUT_STEP),
        (optimization, "gradient_accumulation_steps", PROMPTS_PER_ROLLOUT_STEP),
        (optimization, "ppo_passes", 1),
        (optimization, "max_grad_norm", 1.0),
        (validation, "rows", 200),
        (validation, "interval_rollout_steps", 10),
        (validation, "generation_batch_size", 16),
        (validation, "max_new_tokens", 2048),
        (output, "checkpoint_rollout_steps", 50),
        (output, "keep_checkpoints", 5),
    ):
        _require_canonical(section, key, expected)

    if algorithm_name == "rank_grpo":
        for key, expected in (
            ("return", "exp_inf"),
            ("missing_rank_reward", 0.0),
            ("underflow_penalty", -0.1),
            ("overflow_penalty", -0.1),
        ):
            _require_canonical(algorithm, key, expected)
        _require_canonical(algorithm, "clip_low", 0.06)
        _require_canonical(algorithm, "clip_high", 0.08)
        _require_canonical(optimization, "objective", "paper_rank_grpo")
    else:
        _require_canonical(algorithm, "clip_low", 0.2)
        _require_canonical(algorithm, "clip_high", 0.26)
        _require_canonical(optimization, "objective", "paper_sequence_grpo")
    return algorithm_name


def _load_huggingface_answer_rl_trainer(**kwargs: Any) -> Any:
    """Late optional-dependency boundary, also injectable in CLI tests."""

    from openbench_rerank_rl.answer_trainer import (
        load_huggingface_answer_rl_trainer,
    )

    return load_huggingface_answer_rl_trainer(**kwargs)
...
def _diagnostic_row(query_step: int, result: Any) -> dict[str, object]:
    ...
    completion_count = len(credit.completions)
    segmentation_failures = sum(
        item.segmentation_reason is not None for item in credit.completions
    )
    return {
        ...
        "active_tokens": result.active_token_count,
        "active_actions": result.action_count,
        "parseable_completions": credit.parseable_count,
        "underflow_completions": credit.underflow_count,
        "overflow_items": credit.overflow_count,
        "segmentation_failures": segmentation_failures,
        "ranking_rewards": list(credit.sequence_rewards),
...
def _rollout_tracking_metrics(
    ...
    *,
    ...
    """Reduce one rollout window to scalar metrics with no prompts or row IDs."""
    ...
    completion_count = sum(int(row["completion_count"]) for row in diagnostics)
    active_actions = sum(int(row["active_actions"]) for row in diagnostics)
    rewards = [
        float(value)
        for row in diagnostics
        for value in row["ranking_rewards"]  # type: ignore[union-attr]
    ]
    metrics: dict[str, MetricValue] = {
        ...
        "train/query_pass_1_loss_mean": _mean(
            [float(row["loss"]) for row in diagnostics]
        ),
        "train/ranking_reward": _mean(rewards),
        "train/parse_rate": (
            sum(int(row["parseable_completions"]) for row in diagnostics)
            / completion_count
            if completion_count
            else 0.0
        ),
        "train/underflow_rate": (
            sum(int(row["underflow_completions"]) for row in diagnostics)
            / completion_count
            if completion_count
            else 0.0
        ),
        "train/overflow_items_per_completion": (
            sum(int(row["overflow_items"]) for row in diagnostics)
            / completion_count
            if completion_count
            else 0.0
        ),
        "train/segmentation_failure_rate": (
            sum(int(row["segmentation_failures"]) for row in diagnostics)
            / completion_count
            if completion_count
            else 0.0
        ),
        "train/active_tokens": int(final_pass.active_token_count),
        "train/active_actions": int(
            getattr(final_pass, "action_count", active_actions)
        ),
        "progress/query_steps": completed_queries,
        "progress/original_rollouts": completed_queries * GROUP_SIZE,
        "progress/optimizer_step": optimizer_step,
        "progress/ppo_passes": 1,
    }
...
def _validation_tracking_metrics(
    result: AnswerOnlyValidationResult,
) -> dict[str, MetricValue]:
    return {
        "validation/rows": result.rows,
        "validation/ndcg": result.ndcg,
        "validation/parse_rate": result.parse_rate,
        "validation/bare_list_rate": result.bare_list_rate,
        "validation/valid_unique_ids_rate": result.valid_unique_ids_rate,
...
def _safe_tracking_config(
    *,
    model_name: str,
    algorithm_name: AnswerRLAlgorithm,
    algorithm: Mapping[str, Any],
    ...
    """Whitelist non-sensitive run metadata; raw rows and paths never reach W&B."""

    return {
        "algorithm": algorithm_name,
        "rank_return": (
            str(algorithm.get("return", "not-applicable"))
            if algorithm_name == "rank_grpo"
            else "not-applicable"
        ),
        ...
        "normalization_epsilon": float(algorithm.get("normalization_eps", 1e-4)),
        "normalization_ddof": 1,
        "underflow_penalty": float(algorithm.get("underflow_penalty", -0.1)),
        "overflow_penalty": float(algorithm.get("overflow_penalty", -0.1)),
        "ppo_passes": 1,
        "max_rollout_steps": max_rollout_steps,
        "prompts_per_rollout_step": PROMPTS_PER_ROLLOUT_STEP,
        "originals_per_prompt": GROUP_SIZE,
        "originals_per_rollout_step": ORIGINALS_PER_ROLLOUT_STEP,
        "do_sample": sampling.do_sample,
        "temperature": sampling.temperature,
        "top_k": sampling.top_k,
        "top_p": sampling.top_p,
        "max_new_tokens": sampling.max_new_tokens,
        "generation_batch_size": sampling.original_batch_size,
        "max_slate_size": 20,
        ...
        "starting_adapter_sha256": input_fingerprints["starting_adapter_sha256"],
        "training_source_sha256": input_fingerprints["package_source_sha256"],
        "actor_device_map": json.dumps(
            actor_device_map, sort_keys=True, default=str
        ),
...
def _resolve_resume_checkpoint(
    ...
    *,
    ...
    if value == "latest":
        return latest
    selected = validate_checkpoint(Path(value).expanduser(), output_dir=output_dir)
...
def _validated_resume_state(
    ...
    *,
    ...
    if completed_queries != rollout_step * accumulation_steps:
        raise ValueError("checkpoint is not at a complete prompt-accumulation boundary")
...
def _write_run_info(
    ...
    *,
    ...
    if run_info is None:
        return
    suffix = (
        "" if resume_rollout_step is None else f"-resume-from-{resume_rollout_step:08d}"
    )
...
def main() -> None:
    ...
    algorithm_name = _validate_canonical_config(
        model=model,
        data=data,
    ...
    starting_adapter_path = Path(str(model["adapter_path"])).expanduser().resolve()
    ...
    sampling = SamplingConfig(
        ...
        original_batch_size=int(generation.get("batch_size", 4)),
        counterfactual_batch_size=int(generation.get("batch_size", 4)),
    )
    ...
    accumulation_steps = _positive_int(
        optimization.get("gradient_accumulation_steps", PROMPTS_PER_ROLLOUT_STEP),
        "gradient_accumulation_steps",
    )
    ppo_passes = _positive_int(optimization.get("ppo_passes", 1), "ppo_passes")
    ...
    validation_batch_size = _positive_int(
        validation.get("generation_batch_size", 16),
    ...
    log_query_steps = _positive_int(
        output.get("log_query_steps", 1), "log_query_steps"
    )
    checkpoint_rollout_steps = _positive_int(
        output.get("checkpoint_rollout_steps", 50), "checkpoint_rollout_steps"
    )
    keep_checkpoints = _positive_int(
        output.get("keep_checkpoints", 5), "keep_checkpoints"
    )
    ...
    fingerprints = {
        ...
        "starting_adapter_sha256": sha256_tree(starting_adapter_path),
        "model_revision": model_revision,
    ...
    resume_checkpoint = _resolve_resume_checkpoint(
        args.resume_from_checkpoint, output_dir=output_dir
    )
    resume_state = (
    ...
    wandb_settings = parse_wandb_settings(
        wandb_values, mode_override=args.wandb_mode
    )
    ...
    exit_code = 1
    try:
        trainer = _load_huggingface_answer_rl_trainer(
            model_name_or_path=str(model["name_or_path"]),
            adapter_path=str(starting_adapter_path),
            actor_adapter_path=(
            ...
            rank_metric=str(algorithm.get("rank_metric", "ndcg")),
            clip_low=float(algorithm["clip_low"]),
            clip_high=float(algorithm["clip_high"]),
            beta=float(optimization.get("reference_kl_coefficient", 0.01)),
            normalization_epsilon=float(algorithm.get("normalization_eps", 1e-4)),
            normalization_ddof=1,
            underflow_penalty=float(algorithm.get("underflow_penalty", -0.1)),
            overflow_penalty=float(algorithm.get("overflow_penalty", -0.1)),
        ...
        if resume_checkpoint is not None and resume_state is not None:
            ...
            if (
                ...
                or trainer.optimizer_steps != resume_checkpoint.rollout_step
            ):
        ...
        metadata = {
            ...
            "expected_optimizer_steps": max_rollout_steps,
            "ppo_passes": ppo_passes,
            "num_siblings": GROUP_SIZE,
            "effective_prompt_batch_size": PROMPTS_PER_ROLLOUT_STEP,
            "effective_original_batch_size": ORIGINALS_PER_ROLLOUT_STEP,
            "gradient_accumulation_steps": accumulation_steps,
        ...
        with (
            ...
            def record_completed_rollout(
                ...
                *,
                ...
                checkpoint_saved = (
                    full_rollout and rollout_step % checkpoint_rollout_steps == 0
                )
                if checkpoint_saved:
            ...
            # Restore last: loading, tracking setup, data skipping, log repair,
            # and baseline validation must not alter the next sampling decision.
